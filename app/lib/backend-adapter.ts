...
export async function callBackend(request: BackendRequest): Promise<BackendResponse> {
  const isDirect = process.env.BACKEND_DIRECT_CALL === 'true'

  if (isDirect && typeof window === 'undefined') {
    // Server-side: Direct function call
    return callDirectBackend(request)
  } else {
    // Client-side or fallback: HTTP proxy
    return callHttpBackend(request)
  }
}

/**
 * Direct backend function calls (server-side only)
 */
async function callDirectBackend(request: BackendRequest): Promise<BackendResponse> {
  try {
    const { path, method, body, query, headers = {} } = request

    // Dynamic imports based on path
    if (path.startsWith('/runs')) {
      return await handleRunsEndpoint(path, method, body, query)
    } else if (path.startsWith('/system')) {
      return await handleSystemEndpoint(path, method)
    } else if (path.startsWith('/auth')) {
      return await handleAuthEndpoint(path, method, body)
    } else if (path.startsWith('/worker')) {
      return await handleWorkerEndpoint(path, method, body)
    } else if (path.startsWith('/sam3d')) {
      return await handleSam3dEndpoint(path, method, body)
    } else {
      return { status: 404, data: { error: 'Endpoint not found' }, headers: {} }
    }
  } catch (error) {
    console.error('Direct backend call error:', error)
    return {
      status: 500,
