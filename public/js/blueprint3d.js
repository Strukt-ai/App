    ...
    (function (Items) {
        ...
        var Item = (function (_super) {
            ...
            Item.prototype.resize = function (height, width, depth) {
                ...
                var curD = this.getDepth();
                if (!isFinite(curW) || curW === 0 || !isFinite(curH) || curH === 0 || !isFinite(curD) || curD === 0) {
                    return;
                }
                var x = width / curW;
                var y = height / curH;
            ...
            Item.prototype.updateHighlight = function () {
                ...
                var mats;
                if (this.material && this.material.materials) {
                    mats = this.material.materials;
                }
                ...
                mats.forEach(function (material) {
                    if (material && material.emissive && material.emissive.setHex) {
        ...
        var Scene = (function () {
            ...
            Scene.prototype.addItem = function (itemType, fileName, metadata, position, rotation, scale, fixed) {
                ...
                var objectCallback = function (object) {
                    ...
                    var item = new (BP3D.Items.Factory.getClass(itemType))(scope.model, metadata, geometry, new THREE.MeshFaceMaterial([invisibleMaterial]), position, rotation, autoScale);
                    item.fixed = fixed || false;
                ...
                if (lower.indexOf('.obj', lower.length - 4) !== -1) {
                    if (metadata && metadata.mtlUrl && THREE.OBJMTLLoader) {
                        var objMtlLoader = new THREE.OBJMTLLoader();
                        objMtlLoader.load(fileName, metadata.mtlUrl, function (object) {
        ...
        Three.Controller = function (three, model, camera, element, controls, hud) {
            ...
            function mouseUpEvent(event) {
                if (scope.enabled) {
                    ...
                    switch (state) {
                        case states.DRAGGING:
                            if (selectedObject && selectedObject.clickReleased) {
